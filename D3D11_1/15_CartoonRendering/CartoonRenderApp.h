...
class CartoonRenderApp : public GameApp
{
public:
	CartoonRenderApp(HINSTANCE hInstance);
	~CartoonRenderApp();
	...
	ComPtr<ID3D11RasterizerState>	m_pReverseRasterizerState = nullptr; // face를 뒤집은 레스터라이저 상태
	...
	ComPtr<ID3D11PixelShader> m_pToonRampShader = nullptr;
	ComPtr<ID3D11PixelShader> m_pSolidShader = nullptr;

	// vertex shaderes
	ComPtr<ID3D11VertexShader> m_pRigidMeshVertexShader = nullptr;	
	ComPtr<ID3D11VertexShader> m_pSkinnedMeshVertexShader = nullptr;	
	ComPtr<ID3D11VertexShader> m_pSkinnedMeshOutlineVertexShader = nullptr;	
	...
	Color m_LightColor{ 1,1,1,1 };			// Directional Light의 
	...
	Color m_outLineColor{ 1,1,1,1 };

	float m_Near = 0.01f;
	float m_Far = 1000.0f;
	float m_PovAngle = XM_PIDIV2;

	int psIndex = 3; // 픽셀 셰이더 구별용 // 0. blinn-phong, 1. phong, 2. toon 
