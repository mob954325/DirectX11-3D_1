...
CartoonRenderApp::CartoonRenderApp(HINSTANCE hInstance)
	: GameApp(hInstance)
{

}

CartoonRenderApp::~CartoonRenderApp()
{
	UninitImGUI();
}

bool CartoonRenderApp::OnInitialize()
{
	if (!InitD3D())
		return false;
...
void CartoonRenderApp::OnUpdate()
{
...
void CartoonRenderApp::OnRender()
{
#if USE_FLIPMODE == 1
	...
	cb.outlineColor = m_outLineColor;
	...
	{
		m_pDeviceContext->VSSetShader(m_pSkinnedMeshOutlineVertexShader.Get(), 0, 0);
	}
	m_pDeviceContext->PSSetShader(m_pSolidShader.Get(), 0, 0);
	m_pDeviceContext->RSSetState(m_pReverseRasterizerState.Get());
	...
	case 2:
		...
		break;
	case 3:
		m_pDeviceContext->PSSetShader(m_pToonRampShader.Get(), 0, 0);
		break;
	}
...
bool CartoonRenderApp::InitImGUI()
{
...
void CartoonRenderApp::RenderImGUI()
{
	...
	ImGui::RadioButton("ToonRamp", psIndex == 3); if (ImGui::IsItemClicked()) psIndex = 3;
...
void CartoonRenderApp::UninitImGUI()
{
	// Cleanup
...
bool CartoonRenderApp::InitScene()
{
	HRESULT hr = S_OK;
	...
	m_pDevice->CreateRasterizerState(&rasterizerDesc, &m_pReverseRasterizerState);
...
bool CartoonRenderApp::InitEffect()
{
	...
	vertexShaderBuffer.Reset();
	HR_T(CompileShaderFromFile(L"Shaders\\VS_SkinnedMeshOutline.hlsl", "main", "vs_5_0", vertexShaderBuffer.GetAddressOf()));
	HR_T(m_pDevice->CreateVertexShader(vertexShaderBuffer->GetBufferPointer(), vertexShaderBuffer->GetBufferSize(), NULL, m_pSkinnedMeshOutlineVertexShader.GetAddressOf()));
	...
	HR_T(m_pDevice->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(), pixelShaderBuffer->GetBufferSize(), NULL, m_pSolidShader.GetAddressOf()));
	...
	pixelShaderBuffer.Reset();
	HR_T(CompileShaderFromFile(L"Shaders\\PS_ToonRamp.hlsl", "main", "ps_5_0", pixelShaderBuffer.GetAddressOf()));
	HR_T(m_pDevice->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(), pixelShaderBuffer->GetBufferSize(), NULL, m_pToonRampShader.GetAddressOf()));

	return true;
}

void CartoonRenderApp::ResetValues()
{
	m_Near = 0.01f;
	m_Far = 1000.0f;
...
LRESULT CartoonRenderApp::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
