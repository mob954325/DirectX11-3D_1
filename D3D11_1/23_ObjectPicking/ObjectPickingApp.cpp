...
ObjectPickingApp::ObjectPickingApp(HINSTANCE hInstance)
	: GameApp(hInstance)
{
	gbufferCount = static_cast<int>(EGbuffer::Count);
}

ObjectPickingApp::~ObjectPickingApp()
{
	UninitImGUI();
}

void ObjectPickingApp::InitDebugDraw()
{
...
void ObjectPickingApp::InitShdowMap()
{
...
bool ObjectPickingApp::InitSkyBox()
{
...
void ObjectPickingApp::CreateSwapchain()
{
...
void ObjectPickingApp::DrawFrustum(Matrix worldMat, Matrix viewMat, Matrix proejctionMat,
...
bool ObjectPickingApp::InitDxgi()
{
	// ============================================
	// 1. IDXGIDevice3 생성
	// ============================================
...
void ObjectPickingApp::CreateQuad()
{
...
void ObjectPickingApp::CreateGbuffers()
{
	// gbuffer
...
void ObjectPickingApp::ResizeScreen(int width, int height)
{
	if (!m_hWnd) return;
...
void ObjectPickingApp::ResizeResource()
{
	// Clear
...
void ObjectPickingApp::ResetGBuffers()
{
...
void ObjectPickingApp::OnUpdate()
{
	...
	m_models[1]->m_Scale = m_GroundScale;
...
void ObjectPickingApp::OnRender()
{
	// clear
...
void ObjectPickingApp::RenderPassGBuffer()
{
...
void ObjectPickingApp::RenderPassDirectionalLight()
{
...
void ObjectPickingApp::SkyboxPass()
{
	// 카메라용 뷰 행렬과, 투영행렬
...
void ObjectPickingApp::DepthOnlyPass()
{
	// 바인딩 해제
...
bool ObjectPickingApp::InitImGUI()
{
...
void ObjectPickingApp::RenderImGUI()
{
	...
	if (m_models[0]->modelAsset->animations.size() > 0)
	{
		ImGui::Begin("Middle Model Animation info");
		{
			ImGui::Text(std::to_string(m_models[0]->m_progressAnimationTime).c_str());
			ImGui::Checkbox("isPlay", &m_models[0]->isAnimPlay);
			ImGui::SliderFloat("Animation Duration", &m_models[0]->m_progressAnimationTime, 0.0f, m_models[0]->modelAsset->animations[m_models[0]->m_animationIndex].m_duration);
		}
		ImGui::End();
	}
...

	{
		...
		m_models[0]->m_Position = m_charaPosition;
		m_models[0]->m_Rotation = m_charaRotation;
		m_models[0]->m_Scale = m_charaScale;
	}
	...
	for (auto& mesh : m_models[0]->modelAsset->meshes)
	{
...
void ObjectPickingApp::UninitImGUI()
{
	// Cleanup
...
bool ObjectPickingApp::InitScene()
{
	HRESULT hr = S_OK;
	...
	if (!m_models[0]->Load(m_hWnd, m_device, m_deviceContext, "..\\Resource\\char.fbx"))
	{
		MessageBox(m_hWnd, L"FBX file is invaild at path", NULL, MB_ICONERROR | MB_OK);
	}
	m_models[0]->GetBuffer(m_transformBuffer, m_bonePoseBuffer, m_boneOffsetBuffer);
	m_models[0]->m_Position = { 0, 100, 0 };

	m_models.emplace_back(make_unique<SkeletalModel>());
	if (!m_models[1]->Load(m_hWnd, m_device, m_deviceContext, "..\\Resource\\Ground.fbx"))
	{
		MessageBox(m_hWnd, L"FBX file is invaild at path", NULL, MB_ICONERROR | MB_OK);
	}
	m_models[1]->GetBuffer(m_transformBuffer, m_bonePoseBuffer, m_boneOffsetBuffer);
	m_models[1]->m_Position = { 0, -100, 0 };

	m_models.emplace_back(make_unique<SkeletalModel>());
	if (!m_models[2]->Load(m_hWnd, m_device, m_deviceContext, "..\\Resource\\Tree.fbx"))
	{
		MessageBox(m_hWnd, L"FBX file is invaild at path", NULL, MB_ICONERROR | MB_OK);
	}
	m_models[2]->GetBuffer(m_transformBuffer, m_bonePoseBuffer, m_boneOffsetBuffer);
	m_models[2]->m_Scale = { 100, 100, 100 };
	m_models[2]->m_Position = { 0, 0, 300 };

	m_models.emplace_back(make_unique<SkeletalModel>());
	if (!m_models[3]->Load(m_hWnd, m_device, m_deviceContext, "..\\Resource\\sphere.fbx"))
	{
		MessageBox(m_hWnd, L"FBX file is invaild at path", NULL, MB_ICONERROR | MB_OK);
	}
	m_models[3]->GetBuffer(m_transformBuffer, m_bonePoseBuffer, m_boneOffsetBuffer);
	m_models[3]->m_Position = { 200, 100, 100 };
...
bool ObjectPickingApp::InitEffect()
{
	// Geometry Pass
	D3D11_INPUT_ELEMENT_DESC layout[] =
	{
...
void ObjectPickingApp::ResetValues()
{
	m_Near = 0.01f;
	m_Far = 3000.0f;
...
void ObjectPickingApp::ShowMatrix(const DirectX::XMFLOAT4X4& mat, const char* label)
{
	// 각 행을 출력
...
LRESULT ObjectPickingApp::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
...
void ObjectPickingApp::OnInputProcess(const Keyboard::State& KeyState, const Keyboard::KeyboardStateTracker& KeyTracker, const Mouse::State& MouseState, const Mouse::ButtonStateTracker& MouseTracker)
{
