...
void LightingApp::OnUpdate()
{
	...
	else
	{
		...
		// Cube Position
		m_CubePosition.y += delta;

		position = m_World1.CreateTranslation(m_CubePosition);
		rotate = m_World1.CreateFromYawPitchRoll(m_CubePosition);
		scale = m_World1.CreateScale(m_CubePosition);
...
void LightingApp::RenderImGUI()
{
	...
	// 큐브 위치 조절 
	ImGui::DragFloat3("Cube1 Position", &m_CubePosition.x);
	ImGui::NewLine();
